...
export default function ResultsView({ quiz, answers, settings, onRetry, onNewTopic, onLearnMore, onHistory }: Props) {
  ...
  useEffect(() => {
    addHistoryEntry({ topic: settings.topic, subject: settings.subject, type: 'quiz', quizTitle: quiz.title, score, total, pct });
  ...
  return (
    <div className="min-h-dvh flex flex-col" style={{ background: 'var(--bg)' }}>
      <div className="flex-1 px-6 py-8 max-w-6xl mx-auto w-full view-enter">
        <div className="md:grid md:grid-cols-[300px_1fr] md:gap-10 md:items-start">

          {/* ── Left: score + actions ── */}
          <div className="space-y-5 md:sticky md:top-8">
            {/* Score */}
            <div
            ...
            >
              ...
              <div className="relative w-[120px] h-[120px]">
                <svg width="120" height="120" style={{ transform: 'rotate(-90deg)' }}>
                  <circle cx="60" cy="60" r="52" strokeWidth="7" stroke="var(--border)" fill="none" />
                  <circle
                    cx="60" cy="60" r="52" strokeWidth="7" stroke="var(--accent)" fill="none"
                    strokeLinecap="round"
              ...
              <p className="text-sm" style={{ color: 'var(--text-2)' }}>{score} of {total} correct</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                <GraduationCap size={13} /> Teach me more about this
              </button>
              <button
                onClick={onHistory}
                className="w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-medium transition-all"
              ...
              >
                <History size={13} /> View learning history
              </button>
            </div>
          </div>
                  ...
                  >
                    <Accordion.Header>
                      <Accordion.Trigger className="w-full flex items-start gap-3 px-4 py-3 text-left outline-none cursor-pointer group"
                        ...
                        <ChevronDown size={14} className="flex-shrink-0 mt-0.5 group-data-[state=open]:rotate-180 transition-transform" style={{ color: 'var(--text-3)' }} />
                      </Accordion.Trigger>
                    </Accordion.Header>
                    <Accordion.Content className="overflow-hidden" style={{ background: 'var(--muted)' }}>
                      <div className="px-4 pt-2 pb-3 space-y-1.5">
                        {!isCorrect && userAnswer && (
                          <p className="text-xs" style={{ color: 'var(--error)' }}>Your answer: {userAnswer}</p>
                        )}
                        {!isCorrect && (
                          <p className="text-xs font-medium" style={{ color: 'var(--success)' }}>Correct: {q.options[q.correct_index]}</p>
                        )}
                        <p className="text-xs leading-relaxed" style={{ color: 'var(--text-2)' }}>{q.explanation}</p>
                      </div>
                    </Accordion.Content>
                  </Accordion.Item>
                );
